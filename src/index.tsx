...
const SettingsModal = ({
    ...
    onUserCreate: (email: string, pass: string) => void;
    ...
    const handleUserCreateSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        onUserCreate(newUserEmail, newPassword);
        setNewUserEmail('');
        setNewPassword('');
    };
                    ...
                    {activeTab === 'users' && currentUser?.role === 'admin' && (
                        <div className="settings-section user-management-section">
                            ...
                            <form onSubmit={handleUserCreateSubmit} className="user-management-form">
                                <input type="email" value={newUserEmail} onChange={(e) => setNewUserEmail(e.target.value)} placeholder="New User's Email" required />
                            ...
                            <div className="user-list-table-wrapper">
                                    ...
                                    <tbody>
                                        {userRoles.map(user => (
                                            <tr key={user.email}>
                                                ...
                                                <td>
                                                    {user.role !== 'admin' && (
                                                        <button className="btn-danger" onClick={() => onUserDelete(user.email)} disabled>Delete</button>
                                                    )}
                                                </td>
                                            </tr>
                                        ))}
                             ...
                             <p className="setting-note">
                                <strong>Note:</strong> User deletion is disabled in the UI. This action requires secure, server-side permissions to prevent abuse and cannot be safely performed from the client application.
                            </p>
                        </div>
                    )}
                    {activeTab === 'about' && (
                        <div className="settings-section">
                            ...
                            <p className="setting-note" style={{marginTop: '1rem'}}>Version: 2.0.0 (Firebase Auth)</p>
...
const App = () => {
  ...
  if (!geminiApiKey || !firebaseApiKey) {
    const isGeminiMissing = !geminiApiKey;
    return (
      <div className="fullscreen-error-container">
        <div className="error-panel">
          <h1>Configuration Error: API Key Missing</h1>
          <p>The application requires API keys to function, but one or more could not be found.</p>
          <div className="error-instructions">
            <h2>For Developers & Setup</h2>
            <p>Please create a file named <code>.env</code> in the project's root folder and add the following keys:</p>
            <pre><code>
{isGeminiMissing && `VITE_API_KEY=YOUR_GOOGLE_GEMINI_API_KEY_HERE\n`}
{!firebaseApiKey && `VITE_FIREBASE_API_KEY=...\nVITE_FIREBASE_AUTH_DOMAIN=...\n(etc... all firebase keys)`}
            </code></pre>
            <p className="error-warning">
              After adding the keys, you must restart the Vite development server.
            </p>
          </div>
        </div>
      </div>
  ...
  // Load user roles from localStorage on initial mount
  useEffect(() => {
    try {
      const storedUserRoles = localStorage.getItem('studymaster_users');
      if (storedUserRoles) {
        setUserRoles(JSON.parse(storedUserRoles));
      } else {
        // This is a fallback. The admin user should be set up manually as per instructions.
        const adminUser = { email: 'admin@example.com', role: 'admin' as const };
        localStorage.setItem('studymaster_users', JSON.stringify([adminUser]));
        setUserRoles([adminUser]);
      }
    } catch (e) {
      console.error("Failed to load user roles from localStorage", e);
    }
  }, []);
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((firebaseUser: FirebaseUser | null) => {
      if (firebaseUser && firebaseUser.email) {
        const roleInfo = userRoles.find(u => u.email === firebaseUser.email);
        const userRole = roleInfo ? roleInfo.role : 'student'; // Default to student if not found
  ...
  useEffect(() => {
    if (currentUser) {
      try {
        const storedData = localStorage.getItem(`studymaster_data_${currentUser.uid}`);
        if (storedData) {
  ...
  useEffect(() => {
    if (currentUser) {
        try {
            ...
            localStorage.setItem(`studymaster_data_${currentUser.uid}`, JSON.stringify(dataToSave));
        } catch (error) {
            console.error(`Failed to save data for user ${currentUser.uid}`, error);
        }
    }
  ...
  const handleCreateUser = async (email: string, pass: string) => {
      if (!email.trim() || !pass.trim()) {
          alert("Email and password cannot be empty."); return;
      }
      if (userRoles.some(u => u.email === email)) {
          alert("A user with this email already exists in the role map."); return;
      }
      try {
        await auth.createUserWithEmailAndPassword(email, pass);
        // This will automatically log in the new user, logging the admin out.
        // The onAuthStateChanged listener will handle the new user state.
        // We need to add the new user to our role map.
        const newUserRole: UserRoleInfo = { email, role: 'student' };
        const updatedUserRoles = [...userRoles, newUserRole];
        setUserRoles(updatedUserRoles);
        localStorage.setItem('studymaster_users', JSON.stringify(updatedUserRoles));
        alert(`Student account for ${email} created successfully. You have been logged in as this new user. Please log out and back into your admin account to continue managing users.`);
  ...
  const handleDeleteUser = (emailToDelete: string) => {
      // This is a placeholder. Secure deletion requires a backend.
      alert("User deletion from the client is disabled for security reasons.");
  };
  ...
  const handleSaveDocument = async () => {
    ...
    const handleBackup = () => {
        ...
        try {
            ...
            a.download = `studymaster_backup_${currentUser.email}_${timestamp}.json`;
    ...
    const handleClearAllData = () => {
        if (!currentUser) return;
        if (window.confirm(`This will permanently delete all subjects, chats, and calendar events for the user "${currentUser.email}". This action cannot be undone. Continue?`)) {
            setSubjects([]);
            setEvents([]);
            setNotifications([]);
            setActiveSubjectId(null);
            localStorage.removeItem(`studymaster_data_${currentUser.uid}`);
            setIsSettingsOpen(false);
        }
    };
    ...
    <>
      ...
      <aside className={`sidebar ${isSidebarOpen ? 'open' : ''}`}>
        ...
        <div className="sidebar-footer">
            <span style={{marginRight: 'auto', color: 'var(--text-secondary)', fontSize: '0.8rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={currentUser.email}>User: {currentUser.email}</span>
